...
prefix = r'[A-Za-z]{3}\s[0-9]{1,2}\s[0-9]{1,2}:[0-9]{1,2}:[0-9]{1,2}\.[0-9]{1,3}\sINFO\s'
        ...
        for bw, var in bw_list:
            for delay in delay_list:
                for delay_var in [f'{min(delay / 10, 20):.1f}', f'{min(delay / 5, 30):.1f}']:
                    ...
                    with open(os.path.join(log_folder, log_name + '-ccp.log')) as f:
                        logs = f.readlines()
                        logs = list(map(lambda x: x.strip('\n'), logs))
                        for str in logs:
                            if re.match(header, str):
                                alg = re.match(header, str).group(1)
                                probe_rtt_interval_secs = re.match(header, str).group(2)
                                probe_rtt_interval_nanos = re.match(header, str).group(3)
                    ...
                    plt.legend()
                    plt.subplot(122)
                    plt.plot(elapsed_time[0: index], rate[0: index], 'ro-', label='Rate')
                    plt.plot(elapsed_time[0: index], bottle_rate[0: index], 'b^-', label='BottleRate')
                    plt.title(log_name + ' 10s')
                    plt.xlabel('Time (s)')
                    plt.ylabel('BandWidth (Mbps)')
                    plt.legend()
                    plt.savefig(os.path.join('figures', log_name + '.png'))
